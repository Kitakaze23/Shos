...
export async function GET(
  ...
  try {
    ...
    for (const equipment of project.equipment) {
      const purchasePrice = new Decimal(
      ...
      totalMonthlyDepreciation =
        totalMonthlyDepreciation.plus(monthlyDep);
    }
    ...
    const operatingParams = Array.isArray(project.operatingParams)
      ? project.operatingParams.find((p) => !p.month) ||
        project.operatingParams[0]
    ...
    const operatingHours = new Decimal(
      operatingParams.operatingHoursPerMonth.toString()
    );
    ...
    const totalHours = project.members.reduce(
      (sum, m) =>
        sum.plus(new Decimal(m.operatingHoursPerMonth.toString())),
      new Decimal(0)
    );

    const teamAllocation = project.members.map((member) => {
      let allocatedCost = new Decimal(0);
      const memberHours = new Decimal(
        member.operatingHoursPerMonth.toString()
      );
      const memberShare = new Decimal(
        member.ownershipShare.toString()
      );
      ...
      if (
        ...
        allocatedCost = totalMonthlyCost
          .mul(memberHours)
          .div(totalHours);
      ...
      return {
        ...
        operatingHours:
          member.operatingHoursPerMonth.toNumber(),
