...
// Явно помечаем роут как динамический, чтобы Next не пытался его предрендерить
...
export async function GET(request: NextRequest) {
  try {
    ...
    const errorsByEndpoint = errorMetrics.reduce<
      Record<
        string,
        {
          count: number;
    ...
    return NextResponse.json({
      ...
      performance: {
        ...
        requestsByEndpoint: performanceMetrics.reduce<
