      ...
      async authorize(credentials) {
        ...
        if (user.deletedAtScheduled) {
          const daysUntilDeletion = Math.ceil(
            (user.deletedAtScheduled.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
          )
          if (daysUntilDeletion > 0) {
            throw new Error(`Account scheduled for deletion in ${daysUntilDeletion} days. Contact support to restore.`)
          }
        }
